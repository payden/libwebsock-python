...
static PyObject *onpong_callback = NULL;
...
static PyObject *libwebsockpy_onpong(PyObject *self, PyObject *args);
...
static PyMethodDef LibwebsockMethods[] = {
  ...
  {"onpong", libwebsockpy_onpong, METH_VARARGS, "Set onpong callback"},
...
static int ws_onpong(libwebsock_client_state *state)
{
  PyObject *stateObject = NULL;
  PyObject *arglist;
  ...
  if (onpong_callback && stateObject) {
    arglist = Py_BuildValue("(O)", stateObject);
    PyObject_CallObject(onpong_callback, arglist);
    Py_DECREF(arglist);
  }
...
static PyObject *libwebsockpy_onpong(PyObject *self, PyObject *args)
{
  ...
  if (PyArg_ParseTuple(args, "O:onpong", &temp)) {
    if (!PyCallable_Check(temp)) {
      PyErr_SetString(PyExc_TypeError, "parameter must be callable");
      return NULL;
    }
    Py_XINCREF(temp);
    Py_XDECREF(onpong_callback);
    onpong_callback = temp;
    Py_INCREF(Py_None);
    result = Py_None;
  }
  return result;
